...
const RepresentanteEditForm = ({ representante, onEditFinished, onCancelEdit, fabricantes, marcas }) => {
    ...
    useEffect(() => {
        if (representante) {
            const marcasIds = representante.marcasRepresentadas ? 
                representante.marcasRepresentadas.map(marca => 
                    typeof marca === 'string' ? marca : marca._id
                ) : [];
            ...
            setFormData({
                ...
                cobertura: {
                    provincias: representante.cobertura?.provincias || [],
                    localidades: representante.cobertura?.localidades || []
                },
                direccion: representante.direccion || '',
                telefono: representante.telefono || '',
                telefonoAdicional: representante.telefonoAdicional || '',
            ...
            if (representante.marcasRepresentadas && marcas) {
                const fabricantesIds = [...new Set(
                    representante.marcasRepresentadas.map(marca => {
                        const marcaObj = typeof marca === 'string' ? 
                            marcas.find(m => m._id === marca) : marca;
                        return marcaObj ? marcaObj.fabricante._id || marcaObj.fabricante : null;
                    }).filter(Boolean)
                )];
                setSelectedFabricantes(fabricantesIds);
            }
        }
    }, [representante, marcas]);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await api.put(`/apoderado/representantes/${representante._id}`, formData);
            showSuccess('Representante actualizado con éxito!');
            if (onEditFinished) {
                onEditFinished();
            }
        } catch (err) {
            console.error('Error al actualizar el representante:', err.response?.data);
            showError('Error: ' + (err.response?.data || 'Error al actualizar el representante.'));
        }
    };
...
export default RepresentanteEditForm;